...
from src.core import config  # Подключаем наш конфигурационный файл

router = Router()

@router.message(F.text)
async def handle_text(message: Message):
    """Ловим обычный текст и команды переключения моделей."""
    text = message.text.strip()
    
    # ПЕРЕХВАТ КОМАНДЫ СРАВНЕНИЯ
    ...
    # ПЕРЕХВАТ КОМАНДЫ ПЕРЕКЛЮЧЕНИЯ МОДЕЛЕЙ
    if text.startswith("!model"):
        args = text.split()
        if len(args) < 2:
            # Динамически собираем список из доступных в конфиге моделей
    ...
    # ОБЫЧНЫЙ ТЕКСТ (Запрос к RAG / Общение)
    try:
