...
import re
import json
from src.infrastructure.event_bus import bus, VoiceReceivedEvent, TextReceivedEvent
...
class Orchestrator:
    ...
    async def _run_clerk(self, event: TextReceivedEvent):
        """Запускает сортировщика 00_Inbox."""
    ...
    async def _generate_note(self, event: TextReceivedEvent):
        """Пайплайн: Текст -> LLM -> Пост-обработка -> Obsidian."""
        print(f"[Orchestrator] 📝 Обработка текста от {event.user_id}...")

        # Усиленный промпт с примерами (Few-Shot)
        sys_prompt = (
            "Ты — строгий аналитик базы знаний. Преврати текст в заметку Obsidian.\n"
            "ПРАВИЛО 1: Обязательно оборачивай ключевые имена и технологии в двойные квадратные скобки: [[Имя]], [[Технология]].\n"
            "ПРАВИЛО 2: Выдай ответ СТРОГО в таком формате:\n\n"
            "PROPERTIES: теги через запятую\n"
            "TITLE: Название\n"
            "CONTENT:\n"
            "> [!abstract] Резюме\n"
            "Суть одним абзацем.\n\n"
        ...
        title_match = re.search(r'TITLE:(.*?)(CONTENT:|$)', raw_response, re.IGNORECASE | re.DOTALL)
        if title_match: title = title_match.group(1).strip()

        content_match = re.search(r'CONTENT:(.*)', raw_response, re.IGNORECASE | re.DOTALL)
        if content_match: content = content_match.group(1).strip()
        elif title_match: content = raw_response.replace(title_match.group(0), "").strip()

        # --- ПОСТ-ОБРАБОТКА (Принудительное исправление ошибок LLM) ---
        # Исправляем сломанные Callout'ы
        content = re.sub(r'^>\s*!?abstract', '> [!abstract]', content, flags=re.MULTILINE | re.IGNORECASE)
        content = re.sub(r'^>\s*\[!abstract\]\s*(Резюме)?', '> [!abstract] Резюме', content, flags=re.MULTILINE | re.IGNORECASE)
        ...
        try:
            ...
            print(f"[Orchestrator] ❌ Ошибка: {e}")
            await bot.send_message(chat_id=event.user_id, text=f"❌ Ошибка записи: {e}")
    ...
    async def _process_voice_pipeline(self, event: VoiceReceivedEvent):
        # Заглушка для аудио (с отправкой ответа в ТГ)
        print(f"[Orchestrator] 🚀 Запуск пайплайна для {event.audio_path.name}")
        await asyncio.sleep(2)
        response = f"Я Оркестратор. Принял твое голосовое: {event.audio_path.name}. Скоро подключим Whisper!"
        await bot.send_message(chat_id=event.user_id, text=response)
